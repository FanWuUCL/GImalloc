...
Besides the Shallow Parameters, some other parameters that were hard-coded in a program and cannot be modified directly from external, may have a big impact to the performance of the program. For example, 
\begin{equation}
char* base = (char*)CALL_MORECORE(0+EXPOSE\_4316);
\end{equation}
is executed when \emph{dlmalloc} wants to know the address of the end of current heap at the first time of memory allocation. But the function call $CALL\_MORECORE(s)$ can be used to extend (with a positive $s$) or trim (with a negtive $s$) the current heap and the address of the end of the extended/trimmed heap is returned. Specially, $CALL\_MORECORE(0)$ returns the address of current heap's end without extending or trimming it. If we expose a parameter ($EXPOSE\_4316$) from this function call, we can control how much memory should be pre-allocated. The proper size of pre-allocated memory depends on the speific program using this memory allocator. If it's too large that is even bigger than the actual size the program needs, it's a waste of memory. On the other hand, if it's too small that would be used up very soon by the program, this time-consuming system call $CALL\_MORECORE(s)$ may be called again very soon to extend the heap, thus waste some time. By tuning the exposed parameter $EXPOSE\_4316$, users can adjust the value of it to balance the time and memory consumption to fit their needs. In this paper, we try to discover and expose more of such implicit parameters so that users can control the behaviour of the program from more aspects.
