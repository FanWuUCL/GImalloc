...
\lstset{basicstyle=\ttfamily, language=C, tabsize=2, columns=flexible}
...
\alignauthor Westley Weimer \\
\affaddr{University of Virginia} \\
\affaddr{Virginia, USA} \\
\email{weimer@cs.virginia.edu}
\alignauthor Mark Harman, Yue Jia, Jens Krinke \\
\affaddr{CREST, UCL} \\ 
\affaddr{London, UK} \\
\email{\{mark.harman, yue.jia, j.krinke\}@ucl.ac.uk}
} 
...
%\author[*]{Yue Jia}
%\author[*]{Jens Krinke}
%\affil[*]{CREST, UCL, London, UK \{fan.wu.12, mark.harman, yue.jia, j.krinke\}@ucl.ac.uk}
%\affil[+]{University of Virginia, USA weimer@cs.virginia.edu}
...
These discovered parameters, together with existing (`shallow') parameters, form a search space that we tune using search-based optimisation in a bi-objective formulation that optimises both time and memory consumption.
We implemented our approach and evaluated it on four real-world programs.
The results show that we can improve execution time by 12\% or achieve a 21\% memory consumption reduction in the best cases.
In three subjects, our deep parameter tuning results in a significant improvement over the baseline of shallow parameter tuning,
...
\keywords{Parameter tuning; parameter exposure; memory allocation} % NOT required for Proceedings
